# vm_demo.py — Linear, step-by-step demo for self-managed MySQL on a VM (GCE)
# Prereqs:
...
if not all([VM_DB_HOST, VM_DB_PORT, VM_DB_USER, VM_DB_PASS, VM_DB_NAME]):
    raise RuntimeError("One or more VM_DB_* environment variables are missing")
...
print("[STEP 2] Connecting to DB URL:", db_url.replace(VM_DB_PASS, "*****"))
